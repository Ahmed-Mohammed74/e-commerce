...
const VerifyCode = () => {
  ...
  const verifyCode = async (values) => {
    try {
      ...
      setLooding(false);
      setTimeout(() => {
        router.push("/forgot-password/reset"); // رابط صفحة تغيير الباسورد
      }, 1000);
    } catch (err) {
      setSucessMsg(null);
    ...
    <div className="container mx-auto mt-10 px-4">
      ...
      >
        <input
          ...
          className="block py-3 px-4 w-full text-gray-900 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition duration-200 mb-4"
        ...
        {sucessMsg && (
          <p className="text-green-600 text-center mt-4">{sucessMsg}</p>
        )}
      </form>
    </div>
  );
